...
def predict_single_image(img_path, model_rpn, model_classifier_only, cfg, class_mapping):
    ...
    for cls_num, box in boxes.items():
        boxes_nms = roi_helpers.non_max_suppression_fast(box, overlap_thresh=0.8, max_boxes=32)
...
def predict(args_):
    ...
    model_rpn.load_weights(cfg.last_model_path, by_name=True)
    model_classifier.load_weights(cfg.last_model_path, by_name=True)
