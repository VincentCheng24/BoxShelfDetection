...
def train_kitti():
    ...
    all_images, classes_count, class_mapping = get_data(cfg.simple_label_file)  # parsing the data
    ...
    try:
        ...
        model_rpn.load_weights(cfg.last_model_path, by_name=True, skip_mismatch=True)
        model_classifier.load_weights(cfg.last_model_path, by_name=True, skip_mismatch=True)
    ...
    model_rpn.compile(optimizer=optimizer,
                      loss=[losses_fn.rpn_loss_cls(num_anchors), losses_fn.rpn_loss_regr(
                          num_anchors)])  # sum of two individual losses, lambda can be change in losses.py
    ...
    for epoch_num in range(num_epochs):
        ...
        progbar = generic_utils.Progbar(epoch_length)  # Displays a progress bar.
        ...
        while True:
            try:
                ...
                loss_rpn = model_rpn.train_on_batch(X, Y)  # Runs a single gradient update on a single batch of data.

                P_rpn = model_rpn.predict_on_batch(X)  # Returns predictions for a single batch of samples.
                ...
                if cfg.num_rois > 1:
                    if len(pos_samples) < cfg.num_rois // 2:
                        selected_pos_samples = pos_samples.tolist()  # Return the array as a (possibly nested) list.
                    else:
